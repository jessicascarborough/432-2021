...
Here, we're going to put together a code book, first defining each of our variables in the order they appear in the dataframe. Then, we'll take a look at the numerical description of each of the variables.
...
And here, we'll use `describe` from `Hmisc` to provide a numerical description of each of our variables.  We can see that each of our variables has a reasonable range of values, although `Arthritis` has a surprisingly high prevalence. This is likely due, in part, to the process of converting the variables to age-adjusted prevalence. Additionally, the categorical variables, `Poor_MHealth_Bin`, `Doc_Visit_Quar`, and `No_Leisure_Exer_Quin` have the appropriate number of distinct categories. 
...
Using linear regression, I plan to assess whether the prevalence of self-reported poor physical health for the majority of the month can be predicted using prevalence of visiting the doctor for a checkup, arthritis, stroke, and chronic kidney disease. 

## My Quantitative Outcome


Our quantitative outcome, `Poor_PHealth`, is defined as a county's age-adjusted prevalence of poor physical health for >= 14 days in the last 30 days. We're interested in predicting this variable, as it provides a general assessment from individuals of how they view their physical health. I'm interested in assessing whether keeping up with preventative medicine practices by going to the doctor for a checkup regularly provides an impact on how one view's the physical health.  
